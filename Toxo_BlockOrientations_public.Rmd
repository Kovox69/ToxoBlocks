...
# Enables parallel processing with the max number of cores available, on Windows machines this should do nothing but the code should run fine anyways.
...
print(ht)
```
Note that in the heatmap, the directionality is indicated by superscript numbers, ² indicates head to head, ³ tail to tail orientation, while head to tail orientations are not further indicated.  
